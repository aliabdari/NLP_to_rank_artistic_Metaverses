...
from DNNs import FCNet, ConverterNN
import torch
...
def collate_fn(data):
    # desc
    tmp_desc = [x[0] for x in data]
    # tmp = pad_sequence(tmp_data, batch_first=True)
    desc_ = torch.stack(tmp_desc)
    # scenes
...
def start_train():
    ...
    model_descriptor = FCNet(input_size=512, feature_size=output_feature_size)
    ...
    dataset = DescriptionSceneDatasetV2(data_description_, data_scene_, data_img_, type_model_desc="mean")
